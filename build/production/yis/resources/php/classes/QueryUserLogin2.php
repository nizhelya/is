...
or die('Connect Error (' . $_db->connect_errno . ') ' . $_db->connect_error);
	...
	{ 
		...
		return $key;  
			...
			if(isset($params->remail) && ($params->remail)) {
				...
				if($stmt = $_db->prepare("INSERT INTO YISGRAND.USERS (login,password,surname,firstname,lastname,email,keysend,visit) VALUES (?,?,?,?,?,?,?,?)")){
					  $stmt->bind_param('sssssssi', $_login,$_password,$_surname,$_firstname,$_lastname, $_email,$_keysend,$_visit);
					...
					//  $href='http://is.yuzhny.com/is/php/activation.php?login='.$_login.'&code='.$_keysend.'';	
					 //$href='http://is.yuzhny.com/is/resources/php/activation.php?login='.$_login.'&code='.$_keysend.'';
    ...
    $mail->Host = $this->smtp;  // Specify main and backup SMTP servers
    ...
    //Recipients
    $mail->setFrom('yis@yuzhny.com', 'yis');
    $mail->addAddress($_email, $_login);     // Add a recipient
   // $mail->addAddress('ellen@example.com');               // Name is optional
    $mail->addReplyTo('yis@yuzhny.com', 'yis');
