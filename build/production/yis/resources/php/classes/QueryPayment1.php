...
public function newOplata(stdClass $params)
	{
		...
		*/
		...
		.$this->newOplata.'","'
		.$this->user_id.'",'
		.'@kvartplata, @otoplenie, @podogrev, @voda, @stoki, @ins_id, @success, @msg)';
		
		
		$this->result = $_db->query($this->sql) or die('Connect Error in '.$this->what.'(' .  $this->sql . ') ' . $_db->connect_error);
		
		//$this->sql_callback='SELECT @kvartplata, @otoplenie, @podogrev, @voda, @stoki, @tbo, @ins_id, @success, @msg';
		$this->sql_callback='SELECT @kvartplata, @otoplenie, @podogrev, @voda, @stoki, @ins_id, @success, @msg';
...
$iPay->set_transaction(492,$this->results['otoplenie']+ $this->results['podogrev'],'Отопление и подогрев',4471);
