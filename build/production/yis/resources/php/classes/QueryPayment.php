...
public function newOplata(stdClass $params)
	{
		...
		.$this->newOplata.'","'
		.$this->user_id.'",'
		.'@kvartplata, @otoplenie, @podogrev, @voda, @stoki, @tbo,@ptn, @ins_id,@ipay, @success, @msg)';
							//print($this->sql); 

		
		$this->result = $_db->query($this->sql) or die('Connect Error in '.$this->what.'(' .  $this->sql . ') ' . $_db->connect_error);
		
		$this->sql_callback='SELECT @kvartplata, @otoplenie, @podogrev, @voda, @stoki, @tbo,@ptn, @ins_id, @ipay ,@success, @msg';
		...
		while ($this->row = $this->res_callback->fetch_assoc()) {
			...
			$this->results['ipay']	=$this->row['@ipay'];
...
$iPay->set_urls('https://is.yuzhny.com/is/index.php?pay_id='.$this->results['ins_id'].'','https://is.yuzhny.com/is/index.php?pay_id='.$this->results['ins_id'].'');
$iPay->set_mode('real');

// TRANSACTIONS
if ($this->results['kvartplata'] >0 AND $this->results['ipay'] !=0 ) {

$iPay->set_transaction(492, $this->results['kvartplata'] ,'Квартплата',$this->results['ipay']);
...
$iPay->set_transaction(492, $this->results['tbo'],'Вывоз мусора (ТБО)',685168);
