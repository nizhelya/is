...
//print($_sql);
		    ...
		    case "Appartment":
			  ...
			  $_sql= 'SELECT `raion_id`, `house_id`,`address_id`,`address`,`lift`,`fio` as owner ,`order`,`data` as data_ordera,`privat`,'
				  ...
				  .'(SELECT `start` FROM YIS.DOGOVOR_YTKE WHERE `address_id`='.$_id.' LIMIT 1) AS dog_ytke_data, '
				  .'(SELECT `nomer` FROM YIS.DOGOVOR_VIK WHERE `address_id`='.$_id.' LIMIT 1) AS dog_vik, '
				  .'(SELECT `start` FROM YIS.DOGOVOR_VIK WHERE `address_id`='.$_id.' LIMIT 1) AS dog_vik_data '
...
//print($_sql);  
	...
	//print($_sql);  
		    ...
		    break;
			  ...
			  $_sql='SELECT address_id, address, god, DATE_FORMAT(data,"%d-%m-%Y") as fdate,kvartplata,otoplenie,podogrev,voda,stoki,tbo,summa,prixod,kassa,nomer,operator,data_in  FROM OPLATA  WHERE address_id='.$_id.' ORDER BY data DESC';
//print($_sql);
